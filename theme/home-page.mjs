...
import { partnerWall } from "./components.mjs";
...
export function homePage() {
  const featured = ["shorfat-al-haram", "malqa-taif", "dream-land", "khairat-taibah"].map(getProject).filter(Boolean);
  const storyFrames = [
  ...
  const projectsSection = `<section class="px-work" id="work" aria-labelledby="px-work-title"><div class="shell px-work-intro"><div class="kicker">Selected work</div><h2 id="px-work-title">Proof lives in the <em>launch.</em></h2><p>Campaigns for residential plots, auctions and destination projects across Makkah, Taif and Taibah.</p></div><div class="px-work-list">${featured.map((p, i) => `<article class="px-project" data-project-scene><a href="/work/${p.slug}/" class="shell px-project-link" data-cursor="View"><div class="px-project-media"><img src="${p.image}" alt="${p.alt}" width="1319" height="791" loading="lazy" decoding="async"><span class="px-project-number" aria-hidden="true">0${i + 1}</span></div><div class="px-project-info"><div><span class="kicker">${p.location} · ${p.type}</span><h3>${p.title}</h3></div><div><p class="px-project-services">${p.services.slice(0, 5).join(" · ")}</p><span class="px-project-arrow">View case study ↗</span></div></div></a></article>`).join("")}</div><div class="shell px-work-more"><a class="btn btn-ghost" href="/work/" data-snd>All ${projects.length} case studies ↗</a></div></section>`;
